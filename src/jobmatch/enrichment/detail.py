...
def set_proxy(proxy_str: str | None):
    ...
    if proxy_str:
        from jobmatch.discovery.jobspy import parse_proxy
...
def _load_base_urls() -> dict[str, str | None]:
    ...
    from jobmatch.config import load_base_urls
    ...
    for row in rows:
        ...
        if new_url and new_url != url:
            try:
                ...
                conn.execute(
                    "UPDATE jobs SET status = 'url_merged', status_reason = ? WHERE url = ?",
                    (f"canonical URL collided with {new_url}", url),
                )
                resolved += 1
        else:
            failed += 1
