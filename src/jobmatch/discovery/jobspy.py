...
parses salary ranges, and stores everything in the JobMatch database.
...
def resolve_proxy_config(cfg: dict) -> dict | None:
    """Resolve proxy from search config.
    
    Handles two formats:
    1. String: "host:port:user:pass" or "host:port"
    2. Dict: {"enabled": bool, "host": str, "port": int, "user": str|None, "pass": str|None}
    """
    ...
    if isinstance(proxy_raw, str):
        return parse_proxy(proxy_raw)
    
    if isinstance(proxy_raw, dict):
        if not proxy_raw.get("enabled", False):
            return None
        host = proxy_raw.get("host", "")
        port = str(proxy_raw.get("port", ""))
        user = proxy_raw.get("user") or proxy_raw.get("username")
        passwd = proxy_raw.get("pass") or proxy_raw.get("password")
...
def store_jobspy_results(conn: sqlite3.Connection, df, source_label: str) -> tuple[int, int]:
    ...
    for _, row in df.iterrows():
        ...
        if not company:
            from jobmatch.database import extract_company
        ...
        # Extract posting date if JobSpy provided it
        date_posted = None
        ...
        try:
            ...
            conn.execute(
                ...
                "full_description, application_url, detail_scraped_at, date_posted, discovered_run_id) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (url, title, company, salary, description, location_str, site_label, strategy, now,
                 full_description, apply_url, detail_scraped_at, date_posted, run_id),
            )
            new += 1
...
def _run_one_search(
    ...
    # Per-location Indeed country mapping
    loc_label = s.get("location_label", s["location"].split(",")[0])
    country_indeed = country_indeed_map.get(loc_label, defaults.get("country_indeed", "usa"))
...
def search_jobs(
    ...
    proxy_config = proxy if isinstance(proxy, dict) else (parse_proxy(proxy) if proxy else None)
...
def _full_crawl(
    ...
    # Optional chunking for cron reliability. Hermes cron jobs have bounded
    # runtime; splitting the daily search matrix across several cron ticks is
    # simpler and safer than one giant crawl. 1-based index for shell clarity:
    #   JOBMATCH_DISCOVERY_PARTS=4 JOBMATCH_DISCOVERY_PART=1 ... run discover
    parts_raw = os.environ.get("JOBMATCH_DISCOVERY_PARTS", "").strip()
    part_raw = os.environ.get("JOBMATCH_DISCOVERY_PART", "").strip()
    if parts_raw or part_raw:
        try:
            parts = int(parts_raw or "1")
            part = int(part_raw or "1")
        except ValueError as e:
            raise ValueError("JOBMATCH_DISCOVERY_PARTS and JOBMATCH_DISCOVERY_PART must be integers") from e

        if parts < 1:
            raise ValueError("JOBMATCH_DISCOVERY_PARTS must be >= 1")
        if part < 1 or part > parts:
            raise ValueError("JOBMATCH_DISCOVERY_PART must be between 1 and JOBMATCH_DISCOVERY_PARTS")

        # Deterministic round-robin keeps each part balanced even if query
        # ordering changes. Across all parts, every combo is covered once.
        searches = [s for idx, s in enumerate(searches) if idx % parts == (part - 1)]
        log.info(
            "Discovery chunk: part %d/%d -> %d/%d search combinations",
            part, parts, len(searches), total_searches,
        )

    proxy_config = proxy if isinstance(proxy, dict) else (parse_proxy(proxy) if proxy else None)
    country_indeed_map = search_cfg.get("country_indeed_map", {})
...
def run_discovery(cfg: dict | None = None) -> dict:
    ...
    if not cfg:
        log.warning("No search configuration found. Run `jobmatch init` to create one.")
        return {"new": 0, "existing": 0, "errors": 0, "db_total": 0, "queries": 0}

    proxy = resolve_proxy_config(cfg)
    # `boards` is the user-facing key in ~/.jobmatch/searches.yaml. Keep `sites`
    # as a backwards-compatible alias for older configs.
